...
LIVE_COMPOSE_FILE="${COMPOSE_FILE:-$ROOT_DIR/compose/docker-compose.production.yml}"
...
compose_live() {
  docker compose --env-file "$LIVE_ENV_FILE" -f "$LIVE_COMPOSE_FILE" "$@"
}
...
if [[ ! -f "$LIVE_ENV_FILE" ]]; then
  echo "warning: live boot requested but env file is missing: $LIVE_ENV_FILE" >&2
