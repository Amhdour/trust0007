...
def _request_json(url: str, *, data: bytes | None = None, headers: dict[str, str] | None = None, method: str = "GET", timeout: float = 10.0) -> dict[str, Any]:
    request = Request(url, data=data, headers=headers or {}, method=method)
    with urlopen(request, timeout=timeout) as response:
        return json.loads(response.read().decode("utf-8"))


def _request_text(url: str, *, headers: dict[str, str] | None = None, timeout: float = 30.0) -> tuple[int, str]:
    request = Request(url, headers=headers or {}, method="GET")
    try:
        with urlopen(request, timeout=timeout) as response:
            return int(getattr(response, "status", 200)), response.read().decode("utf-8")
...
def main() -> int:
    ...
    launch_status, launch_html = _request_text(
    ...
    overview = _request_json(f"{args.control_plane_base_url.rstrip('/')}/api/control-plane/overview")
