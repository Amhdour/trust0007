...
def test_live_artifact_detection_rejects_demo_summary(tmp_path: Path) -> None:
    ...
    for name in (
        ...
        _write_json(artifacts / name, {"generated_at": _now_iso()})
    _write_json(
        artifacts / "governed-flow-summary.json",
        {"evidence_mode": "demo", "generated_at": _now_iso()},
    )
...
def test_live_artifact_detection_accepts_current_live_summary(tmp_path: Path) -> None:
    ...
    artifacts.mkdir(parents=True, exist_ok=True)
    (artifacts / "events.jsonl").write_text('{"event_type":"request.start"}\n', encoding="utf-8")
    _write_json(artifacts / "identity-evidence.json", {"generated_at": _now_iso(), "authenticated": True, "live": True})
    _write_json(artifacts / "policy-evidence.json", {"generated_at": _now_iso(), "allow": True, "engine": "opa", "engine_reachable": True})
    _write_json(
        artifacts / "retrieval-evidence.json",
        {"generated_at": _now_iso(), "allow": True, "live_backend": True, "backend_verified": True},
    )
    _write_json(artifacts / "trace-correlation.json", {"generated_at": _now_iso(), "complete": True})
    _write_json(artifacts / "launch-gate-result.json", {"generated_at": _now_iso()})
    _write_json(
    ...
    _write_json(
        artifacts / "governed-flow-summary.json",
        {"evidence_mode": "live", "generated_at": _now_iso()},
    )
...
def test_live_artifact_detection_exposes_reason_codes_for_incomplete_live_proof(tmp_path: Path) -> None:
    ...
    _write_json(
        artifacts / "retrieval-evidence.json",
        {"generated_at": _now_iso(), "allow": False, "live_backend": False, "backend_verified": False},
    )
    _write_json(artifacts / "trace-correlation.json", {"generated_at": _now_iso(), "complete": True})
    _write_json(artifacts / "launch-gate-result.json", {"generated_at": _now_iso()})
    _write_json(
    ...
    _write_json(artifacts / "governed-flow-summary.json", {"evidence_mode": "live", "generated_at": _now_iso()})
